	...
	//	GameObject.Find ("Player").transform.rotation = Quaternion.RotateTowards (GameObject.Find ("Player").transform.rotation, test, 9000);
		...
		while (true) {	
			...
			collisionLine = new Line(GameObject.Find ("Player").transform.position, 
			                         GameObject.Find ("Player").transform.position + Vector3.Normalize(dir)*10);
			...
			if(IntersectsWithAnyLine(collisionLine)) {
				...
				if(newDir == Vector3.zero) {
                    ...
                    randomNumber = RNG.Next(4);
                    if(randomNumber == 0)
						current += car.transform.forward;
					if(randomNumber == 1)
						current -= car.transform.forward;
					if(randomNumber == 2)
						current += car.transform.right;
					if(randomNumber == 0)
						current -= car.transform.right;
